...
vec4 lightPos = {1.2f, 1.0f, 2.0f, 1.0f}; // Position of the light source
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glUniform3fv(glGetUniformLocation(shaderCube, "light.specular"), 1,
                 (vec3){1.0f, 1.0f, 1.0f});
    glUniform4fv(glGetUniformLocation(shaderCube, "light.position"), 1,
                 lightPos);
    glUniform1f(glGetUniformLocation(shaderCube, "light.constant"), 1.0f);
    glUniform1f(glGetUniformLocation(shaderCube, "light.linear"), 0.09f);
    glUniform1f(glGetUniformLocation(shaderCube, "light.quadratic"), 0.032f);
    glUniform1f(glGetUniformLocation(shaderCube, "light.cutoff"),
                cos(glm_rad(12.5f)));
    glUniform1f(glGetUniformLocation(shaderCube, "light.outerCutOff"),
                cos(glm_rad(17.5f)));
